...
class Ui_Dialog(object):
    ...
    # Reverse the order of odd sub-arrays
    ...
    def encryptGeometric(self):
        message = self.plainText.toPlainText().lower()
        ...
        if(message==""):
            ...
            completerow = int(len(message) / key)
            lastrow = len(message) % key
    ...
    def fullMatrix(self, array, row):
        message = self.cipheredTextField.toPlainText().lower()
        message = ''.join(message.split())
        key = int(self.cipheredKey.toPlainText())
        for i in range(len(array)):
            if len(message) % key > 0 and len(array[i]) != row:
                array[i].append('null')
        f = []
        for x in range(row):
            for y in range(key):
...
"padding:3px;")
        ...
        #############################################
        self.encrypt.clicked.connect(self.encryptGeometric)
        ##############################################
...
"padding:10px;")
        ...
        self.decrypt.clicked.connect(self.decryptGeometric)
...
"background-color: rgb(255, 255, 255);")
    ...
    def retranslateUi(self, Dialog):
        ...
        self.label_2.setText(_translate("Dialog", "Geometric Cipher"))
